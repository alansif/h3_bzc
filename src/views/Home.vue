<template>
  <div>
    <v-toolbar dense>
      <v-btn small color="primary" @click="addReccord()"><v-icon small left>add</v-icon>添加</v-btn>
      <v-spacer></v-spacer>
      <v-btn small color="primary" @click="$refs.grid.save()"><v-icon small left>done</v-icon>保存</v-btn>
    </v-toolbar>
    <Grid ref="grid"/>
    <v-snackbar v-model="snackbar" color="error" :timeout="3000" top>请先保存后再添加
      <v-btn flat @click="snackbar = false"><v-icon small right>not_interested</v-icon></v-btn>
    </v-snackbar>
  </div>
</template>
